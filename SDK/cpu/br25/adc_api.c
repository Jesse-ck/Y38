...
#if (TCFG_ADKEY_LED_IO_REUSE && TCFG_ADKEY_ENABLE && TCFG_PWMLED_ENABLE)//降低adkey的采样频率，避免led出现闪烁

    static u8 key_cnt = 4;
    if (adc_queue[next_ch].ch == TCFG_ADKEY_AD_CHANNEL) {
        key_cnt++;
        if (key_cnt >= 4) {
            key_cnt = 0;
        } else {
            next_ch = adc_get_next_ch(next_ch);
        }
    }
#endif
...
void _adc_init(u32 sys_lvd_en)
{
    ...
    /* void adc_test();                              */
    /* sys_s_hi_timer_add(NULL, adc_test, 101); //2ms */
