...
SD0_PLATFORM_DATA_BEGIN(sd0_data)
    ...
    .power                  = sd_set_power,//库函数，使用SDPG(PB8)引脚。可以自行重写其他的SD卡电源控制函数，传入0：关电源。传入1，开电源。如果电源硬件已固定不可控，则该函数无效，可以填NULL
    /* .power                  = NULL, */
...
const struct slidekey_port slidekey_list[] = {
	...
	{
		.io         = SLIDE_KEY1_IO,
		.ad_channel = SLIDE_KEY1_AD_CHANNEL,
#if SLIDE_KEY1_MSG == SLIDE_MSG1
		.msg        = USER_MSG_SLIDE1,
#elif SLIDE_KEY1_MSG == SLIDE_MSG2
		.msg        = USER_MSG_SLIDE2,
#endif
		.max_ad     = SLIDE_KEY1_MAX_AD,
		.min_ad     = SLIDE_KEY1_MIN_AD,
		.max_level  = SLIDE_KEY1_MAX_LEVEL,
	},
#endif
	...
	{
		.io         = SLIDE_KEY2_IO,
		.ad_channel = SLIDE_KEY2_AD_CHANNEL,
#if SLIDE_KEY2_MSG == SLIDE_MSG1
		.msg        = USER_MSG_SLIDE1,
#elif SLIDE_KEY2_MSG == SLIDE_MSG2
		.msg        = USER_MSG_SLIDE2,
#endif
		.max_ad     = SLIDE_KEY2_MAX_AD,
		.min_ad     = SLIDE_KEY2_MIN_AD,
		.max_level  = SLIDE_KEY2_MAX_LEVEL,
	}
#endif

};

const struct slidekey_platform_data slide_key_data = {
	.enable = TCFG_SLIDE_KEY_ENABLE,
...
static void board_devices_init(void)
{
    ...
    extern u32 _sdfile_rec_init(void);
    extern int set_rec_capacity(int capacity); //需要先设置容量。
    set_rec_capacity(512*1024);
    _sdfile_rec_init();
#endif
...
#if FLASH_INSIDE_REC_ENABLE
    extern int sdfile_rec_ops_init(void);
    sdfile_rec_ops_init();
#endif
